...
class ThreadClass(threading.Thread):
	...
	def __init__(self, threadName, id, queue):
	...
	def run(self):
		startAndMonitorThread()


previous_image = None
count_nonZero = float('inf')
isDroneRunning = False
warningLimit = 0
droneThreadQueue = None
mythread = None
drone = None
lck = threading.Lock()
...
######################################=-
	...
	def startup(self):
		...
		global isDroneRunning, warningLimit, droneThreadQueue
		global mythread
		droneThreadQueue = Queue.Queue(100)
		isDroneRunning = False
		warningLimit = 0
		mythread = ThreadClass("thread1", 1, droneThreadQueue)
	...
	def shutdown(self):
		...
		print "video pipe sending uninit"
		t=time.time()
...
def vCapture(VidPipePath, parent_pipe):
	import cv2
	global vCruns, commitsuicideV, showVid, lockV, debugV, previous_image, count_nonZero
			...
			if success:
				...
				if count > 0:
					...
					if show:
						...
						(previous_image, count_nonZero)  = guestureRecognise(image, previous_image, count_nonZero)
						key=cv2.waitKey(1)
		...
		for ip in in_pipe:
			if ip == parent_pipe:
				...
				if cmd == "die!":
					...
					print "prabhanjan in foundcodec"
					foundCodec		= True
					burstFrameCount = 0
				elif cmd == "reset" and not reset:# and resetCount<3:
					print "prabhanjan in reset"
					...
					print "initing prabhanjan 12345"
					preinited = True
					try:
						print "trying initing prabhanjan 12345"
					...
					if not inited and reset:
						try:
							print "prabhanjan in here yo!"
							os.mkfifo(VidPipePath)
							os.chmod(VidPipePath, 1411)		# dec => oct : 777 => 1411 | 666 => 1232
						except:
							print "prabhanjan in here except 2 yo!"
							saveVideo = True						# ... fall back to savemode
							parent_pipe.send("saveVideo")			# Inform the main process
							unsureMode = False
							foundCodec = True
			...
			if ip == vstream_pipe:
				...
				if lenVideoPackage == 0:
					print "prabhanjan recieved 0 size packet"
					commitsuicideV = True
				else:
					if inited and not reset:
...
#							print "Boost IN", len(VidStreamSnippet)
						...
						# Just show the I-Frame for slow-video-mode (and repeat for less delay in midVideo()-mode)
							...
							for i in range(0,frameRepeat):			write2pipe.write(rawVideoFrame)	; print " prabhanjan ----"
						# Save-Mode
...
count = 0
#
# def startAndMonitorThread():
# 	global isDroneRunning
# 	global warningLimit
# 	global mythread
# 	global count_nonZero
# 	global drone
# 	try:
# 		if(isDroneRunning == True):
# 			return
# 		print "inside thread handler"
# 		if(2 == 2):
# 			if(2 < 2):
# 				warningLimit = warningLimit + 1
# 			else:
# 				lck.acquire(1)
...
# 				lck.release()
# 		else:
# 			while(isDroneRunning and count_nonZero > 630000 and warningLimit <= 50):
# 				print "keeping the drone active"
# 				if(warningLimit < 50):
# 					lck.acquire(1)
# 					warningLimit = warningLimit + 1
# 					print "warning -> " + str(warningLimit)
# 					lck.release()
# 				else:
# 					print "shutting down drone assuming the drone comes near me"
...
def guestureRecognise(image, previous_image, count_nonZero):
	global isDroneRunning
	global mythread
	global warningLimit
	global count
	...
	img = np.copy(image)
	cv2.rectangle(img,(640,480),(0,0),(0,255,0),0)
	crop_img = img[0:480, 0:300]
	grey = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
	value = (3, 3)
	blurred = cv2.GaussianBlur(grey, value, 0)
	if (previous_image != None):
		img = cv2.absdiff(previous_image, img)

	previous_image = np.copy(image)
	_, thresh1 = cv2.threshold(blurred, 0, 255, cv2.THRESH_OTSU)
	...
	count_nonZero1 = np.count_nonzero(img)

	if(isDroneRunning == False and count_nonZero1 > 630000 and count_nonZero < float('inf')):
        ...
        if count == 0:
            ...
            mythread.threadQueue.put(count_nonZero1)

	return (previous_image, count_nonZero1)